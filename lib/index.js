...
var myUtil = {
      ...
      .then(function (doc) {
        if (!doc) {
          ...
          entity
            .save()
            .then(function (doc) {
              logger.info(fetchTimes,'extracted', queueItem.url, ' saved');
              cb(null);
            })
            .catch(function (e) {
              logger.error(fetchTimes, 'extracted',queueItem.url, " save error");
              cb(e);
            });
        }
        else {
          logger.info(fetchTimes, 'extracted', queueItem.url, ' finded');
          cb(null);
        }
      }
    )
      .catch(function (e) {
        logger.error(fetchTimes, 'extracted',queueItem.url, " find error");
        cb(e);
      });
  },
  createCrawler: function () {
    ...
    crawler.on("fetchcomplete", function (queueItem, responseBuffer, response) {
      ...
      //logger.log(fetchTimes, 'fetched', queueItem.url, crawler.proxyHostname)
    ...
    crawler.on("complete", function () {
      db.close();
      etime = Date.now();
      logger.info(Date.now(),"complete:", (etime - stime) / 1000 / 60 / 60, '小时');
    });

    crawler.on("stop", function () {
      db.close();
      logger.info(Date.now(),"stop");
    });
