...
var myCrawler = {
            ...
            .then(function (doc) {
                if (!doc) {
                    ...
                    entity
                        .save()
                        .then(function (doc) {
                            logger.info(fetchTimes, queueItem.url, 'save ok');
                            cb(null);
                        })
                        .catch(function (e) {
                    ...
                    if (allowUpdate) {
                        ...
                        model
                            ...
                            .then(function (doc) {
                                logger.info(fetchTimes,  queueItem.url, 'update ok');
                                cb(null);
                            })
                            .catch(function (e) {
    ...
    createCrawler: function () {
        ...
        crawler.on("fetchcomplete", function (queueItem, responseBuffer, response) {
            logger.debug(fetchTimes,  queueItem.url,'fetched', 'UserAgent:', crawler.userAgent);
                ...
                if (isTarget) {
                    ...
                    tasks.push(function (cb) {
                        ...
                        try {
                            ...
                            logger.error(fetchTimes, queueItem.url, 'handler error:',e.message);
                            cb(null);
                        }
                    });
        ...
        crawler.on("fetchheaders", function (queueItem, resp) {
            logger.debug(fetchTimes, queueItem.url,'fetchheaders');
        });

        crawler.on("fetchdataerror", function (queueItem, resp) {
            logger.error(fetchTimes, queueItem.url,'fetchdataerror');
        });
        ...
        crawler.on("fetcherror", function (queueItem, resp) {
            logger.error(fetchTimes, queueItem.url,'fetcherror');
        });
        crawler.on("fetch404", function (queueItem, resp) {
            logger.error(fetchTimes,queueItem.url, 'fetch404');
        });
        crawler.on("fetch410", function (queueItem, resp) {
            logger.error(fetchTimes,queueItem.url, 'fetch410');
        });

        crawler.on("fetchclienterror", function (queueItem, resp) {
            logger.error(fetchTimes, queueItem.url,'fetchclienterror');
        });
