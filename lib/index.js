...
var myUtil = {
      ...
      .then(function (doc) {
        if (!doc) {
          ...
          entity
            .save()
            .then(function (doc) {
              console.log(fetchTimes,'extracted', queueItem.url, ' saved');
              cb(null);
            })
            .catch(function (e) {
              console.error(fetchTimes, 'extracted',queueItem.url, " save error");
              cb(e);
            });
        }
        else {
          console.log(fetchTimes, 'extracted', queueItem.url, ' finded');
          cb(null);
        }
      }
    )
      .catch(function (e) {
        console.error(fetchTimes, 'extracted',queueItem.url, " find error");
        cb(e);
      });
  },
  createCrawler: function () {
    ...
    db = mongoose.createConnection(conf.url);
    ...
    crawler.on("fetchcomplete", function (queueItem, responseBuffer, response) {
      ...
      //console.log(fetchTimes, 'fetched', queueItem.url, crawler.proxyHostname)
        ...
        if (handlers[index].target.test(queueItem.url)) {
          ...
          tasks.push(function (cb) {
            ...
            try {
              ...
              if (data) {
                myUtil.saveData(queueItem, data, extractor.keys, extractor.model, cb);
              }
            } catch (e) {
              cb(e);
            }
      ...
      //console.log(fetchTimes,queueItem.url);
    ...
    crawler.on("complete", function () {
      db.close();
      etime = Date.now();
      console.log("complete:", (etime - stime) / 1000 / 60 / 60, '小时');
    });
